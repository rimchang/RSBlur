...
if __name__ == '__main__':
  ...
  for i, values in enumerate(files):
      ...
      if tar_path in no_sat_list:
          non_sat_psnrs.append(psnr[i])
          non_sat_ssims.append(ssim[i])
      elif tar_path in sat_list:
          sat_psnrs.append(psnr[i])
          sat_ssims.append(ssim[i])        
      else:
  ...
  avg_psnr = sum(sat_psnrs)/len(sat_psnrs)
  avg_ssim = sum(sat_ssims)/len(sat_ssims)

  txt = 'For {:s} dataset PSNR on sat images: {:f} SSIM: {:f}\n'.format(input_dir, avg_psnr, avg_ssim)
  print(txt)
  txt_list.append(txt)
  
  avg_psnr = sum(non_sat_psnrs)/len(non_sat_psnrs)
  avg_ssim = sum(non_sat_ssims)/len(non_sat_ssims)

  txt = 'For {:s} dataset PSNR on non sat images: {:f} SSIM: {:f}\n'.format(input_dir, avg_psnr, avg_ssim)
  print(txt)
  txt_list.append(txt)
